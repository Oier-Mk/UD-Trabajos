 ...
 * 
 * EL ARCHIVO HA SIDO MODIFICADO POR mentxaka
 *
 */
public class VentanaGrafica {
	public JFrame ventana;         // Ventana que se visualiza
	boolean cerrada;        // Lógica de cierre (false al inicio)
	JPanel panel;           // Panel principal
	JLabel lMens;           // Etiqueta de texto de mensajes en la parte inferior
	BufferedImage buffer;   // Buffer gráfico de la ventana
	Graphics2D graphics;    // Objeto gráfico sobre el que dibujar (del buffer)
	Point pointPressed;     // Coordenada pulsada de ratón (si existe)
	boolean botonIzquierdo; // Información de si el último botón pulsado es izquierdo o no lo es
	Point pointMoved;       // Coordenada pasada de ratón (si existe)
	Point pointMovedPrev;   // Coordenada pasada anterior de ratón (si existe)
	boolean dibujadoInmediato = true; // Refresco de dibujado en cada orden de dibujado
